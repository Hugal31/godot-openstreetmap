...
class ConvexRoofs:
	...
	func add(polygon, height, roof_angle, color = Color(1, 1, 1)):
		...
		for i in range(s):
			...
			var orthogonal = (p2_3 - p1_3).normalized()
			orthogonal = PackedFloat32Array([orthogonal.x, orthogonal.y, orthogonal.z, 1])
			vertices.append(p1_3)
			normals.append(normal)
			tangents.append_array(orthogonal)
			uvs.append(Vector2(0, 0))
...
class Roofs:
	...
	func add(polygon, height, roof_angle, color = Color(1, 1, 1)):
		...
		for r in range(roofs.size()):
			...
			var orthogonal = (Vector3(u_axis.x, 0, u_axis.y)).normalized()
			var normal = Vector3(0, 1, 0).rotated(orthogonal, roof_angle)
			orthogonal = PackedFloat32Array([orthogonal.x, orthogonal.y, orthogonal.z, 1])
			for i in range(indexes.size()):
