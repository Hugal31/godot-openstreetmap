...
func _draw():
	...
	if data == null:
		return
	# antialiasing was removed
	if data.has("water"):
		for w in data.water:
			draw_colored_polygon(w, Color(0, 0, 1), empty_vec2_array)
	if data.has("grass"):
		for g in data.grass:
			draw_colored_polygon(g, Color(0, 1, 0), empty_vec2_array)
	if data.has("buildings"):
		for g in data.buildings:
			draw_colored_polygon(g.points, Color(0, 0, 0), empty_vec2_array)
	if data.has("roads"):
		for r in data.roads:
			...
			for j in range(point_count):
				...
				if j != 0:
					...
					polygon.append(a2-n2)
					draw_colored_polygon(polygon, Color(1, 0, 0), empty_vec2_array)
				a2 = a1
				n2 = n1
